...
def compute_codebleu(hypothesis, references, lang, params='0.25,0.25,0.25,0.25'):
    ...
    kw_file = root_directory.joinpath(f"CodeBLEU{_split}keywords{_split}{lang}.txt")
    if CURRENT_OS == 'Windows':
        kw_file = str(kw_file)
...
if __name__ == '__main__':
    # main()
    code_response_path = './Design_aware_code_generation/FaCG/CGGeneration/output_model/model_code.json'
    with open(code_response_path,'r',encoding='utf-8') as f:
        jsons = json.load(f)
        for i in range(len(jsons)):
            preds = jsons[i]['response']
            if preds == '':
                score = 0.0
                jsons[i]['codebleu'] = score
                continue
            preds = [preds]
            references = jsons[i]['ground_truth']
            references=[[references]]
            score = code_bleu(preds,references,'java')
            jsons[i]['codebleu'] = score['codebleu']
            if score['codebleu'] == 25.0:
                print(preds)
                print(references)
        with open(code_response_path,'w',encoding='utf-8') as out:
            json.dump(jsons,out)
